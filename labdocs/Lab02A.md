...
1. In [Azure Machine Learning studio](https://ml.azure.com), on the **Compute** page, on the **Compute clusters** tab, click the name of your **aml-cluster** compute cluster you created previously.
...
2. In the **Settings** pane, change the default pipeline name (**Pipeline-Created-on-*date***) to **Visual Retail Sales Training** (if the **Settings** pane is not visible, click the **&#9881;** icon next to the pipeline name at the top).
...
4. On the left side of the designer, expand the **Datasets** section, and drag the **retail sales dataset** dataset you created in the previous exercise onto the canvas.
5. Select the **retail sales dataset** module on the canvas, and view its settings (the settings pane for the dataset may open automatically and cover the canvas). Then on the **outputs** tab, click the **Visualize** icon (which looks like a column chart).
...
1. In the pane on the left, expand the **Data Transformation** section, which contains a wide range of modules you can use to transform data and pre-process it before model training. Drag a **Normalize Data** module to the canvas, below the **retail sales dataset** module. Then connect the output from the **retail sales dataset** module to the input of the **Normalize Data** module.
...
2. The model we're training will predict the **PAYMENT** value, so select the **Train Model** module and modify its settings to set the **Label column** to  **Diabetic** (matching the case and spelling exactly!)
3. The **PAYMENT** label the model will predict is a binary column (cash payment, credit card (magcard) payment), so we need to train the model using a *classification* algorithm. Expand the **Machine Learning Algorithms** section, and under **Classification**, drag a **Two-Class Logistic Regression** module to the canvas, to the left of the **Split Data** module and above the **Train Model** module. Then connect its output to the **Untrained model** (left) input of the **Train Model** module.
